...
@export_custom(PROPERTY_HINT_NONE, "suffix:m/s") var top_speed = 25
@export_custom(PROPERTY_HINT_NONE, "suffix:m/s") var top_reverse_speed = 10
@export_custom(PROPERTY_HINT_NONE, "suffix:m/s/s") var acceleration = 10
@export var roll_resistance = 0.4
@export var break_resistance = 0.2
...
func get_relative_speed() -> float:
	return velocity.length() / top_speed * sign(velocity.dot(-global_transform.basis.z))
...
func _process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("move_jump") && wish_steering != 0:
		wish_drift = true
...
func _physics_process(delta: float) -> void:
	...
	steering = lerp(steering, steer_target, pow(0.2, 5 * delta))
	_update_wheel_rotation()
	
	var turn_speed = deg_to_rad(2) * wish_steering
	if is_on_ground():
		if wish_break && abs(relative_speed) < 1e-1:
			turn_speed = deg_to_rad(5) * wish_steering
		else:
			if wish_drift:
				turn_speed = deg_to_rad(20) * (wish_drift_direction * 0.25 + 0.75 * wish_steering)
			elif abs(relative_speed) > 1e-2:
				turn_speed = deg_to_rad(10) * wish_steering * sign(relative_speed)
	rotate_y(-delta * turn_speed * 5)
