...
func _process(delta):
	...
	if water_material:		
		water_material.set_shader_parameter("ripples_texture_offset", Vector2(texture_pixel_offset) / offset_scalar)
...
func _initialize_compute_code(texture_resolution):
	...
	# Create our shader.
	var shader_file = load("res://water/ripples/water_ripples_compute.glsl")
